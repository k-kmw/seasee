...
function displayArea(coordinates, name, gucode) {
  ...
  $.each(coordinates[0], function (index, coordinate) {
    //console 보면 [0]번째에 배열이 주로 저장이 됨.  그래서 [0]번째 배열에서 꺼내줌.
...
function addMarker(position, title, contentid, contenttypeid) {
  ...
  if (
    ...
    if (
      markersArr.findIndex(function (item) {
        return item.contentId === contentid;
      }) > -1
    ) {
      //markersArr에도 존재한다면?
  ...
  daum.maps.event.addListener(marker, "mouseover", function () {
    ...
    if (!selectedMarker || selectedMarker != marker) {
      ...
      if ($.inArray(marker, markersArr) != -1) {
        return false;
      }
  ...
  daum.maps.event.addListener(marker, "click", function () {
    ...
    var outputTitle =
      ...
      '" text-align:left>';
    outputTitle +=
      '<h4 class="title"><img src="resources/image/pathsidex.png" class= "xbtn" id="d' +
      contentid +
      '" onclick="deletePin(this)">' +
      "<a href='javascript:panTo(" +
      position.jb +
      "," +
      position.ib +
      "," +
...
function wrapWindowByMask() {
  ...
  var left =
    $("#map").scrollLeft() + ($("#map").width() - $("#window").width()) / 2;
  var top =
    $("#map").scrollTop() + ($("#map").height() - $("#window").height()) / 2;
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(afterpath, function (i, val) {
        ...
        sidePath +=
          '<div class="pinn" id="' + val.contentId + '"text-align:left>';
        sidePath +=
          '<h4 class="title"><img src="resources/image/pathsidex.png" class= "xbtn" id="' +
          val.contentId +
          '"onclick="deletePin(this)">' +
          "<a href='javascript:panTo(" +
          val.mapy +
...
function deletePin(event) {
  ...
  if (i > -1) {
    ...
    console.log(cid);
    console.log(
      afterpath.findIndex(function (item) {
        return item.contentId === cid;
      })
    );
    if (
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var output =
        ...
        '" text-align:left>';
      output +=
        '<div class="pin-top"><div id="title" style="background-image: url(\'' +
        "resources/image/popup-top.png" +
        '\');"><font size="4em">' +
        addr.title +
        '</font><a id="popup-X" href="#" onclick="popupX()">';
      output += '<img src="resources/image/popup-X.png"></a></div></div>';
      output +=
        '<div class="scope" ><img src = "' +
        addr.firstimage +
        '" style="height: 250px; width: 350px; float:left"/>';
      if (contenttypeid == 39) {
        output +=
          '<p>네이버 플레이스 음식점 평점 : <div class="glyphicon glyphicon-star"/>' +
          addr.scope +
          "</p>";
      }
      if (addr.image1 && addr.link1) {
        output +=
          '<p class="p">리뷰보기:(클릭으로 이동) </p><a class="review" href="' +
          addr.link1 +
          '"><img src="' +
          addr.image1 +
          '"/>' +
      ...
      if (contenttypeid == 12) {
        ...
        if (myItem.usetimeculture) {
          output +=
            '<p class="p" >' + "운영시간 : " + myItem.usetimeculture + "</p>";
        }
        if (myItem.restdateculture) {
          output +=
            '<p class="p" >' + "휴무일 : " + myItem.restdateculture + "</p>";
        }
        if (myItem.infocenterculture) {
          output +=
            '<p class="p" >' + "연락처 : " + myItem.infocenterculture + "</p>";
        }
      } else if (contenttypeid == 15) {
        if (myItem.eventplace) {
          output +=
            '<p class="p" >' + "행사 장소 : " + myItem.eventplace + "</p>";
        }
        if (myItem.eventstartdate) {
          output +=
            '<p class="p" >' +
            "행사 일정 : " +
            myItem.eventstartdate +
            "~" +
            myItem.eventenddate +
            "</p>";
        }
        if (myItem.playtime) {
          output +=
            '<p class="p" >' + "행사 시간 : " + myItem.playtime + "</p>";
        }
        ...
        if (myItem.usetimeleports) {
          output +=
            '<p class="p" >' + "운영시간 : " + myItem.usetimeleports + "</p>";
        }
        if (myItem.infocenterleports) {
          output +=
            '<p class="p" >' + "연락처 : " + myItem.infocenterleports + "</p>";
        }
      } else if (contenttypeid == 32) {
        if (myItem.reservationurl) {
          output +=
            '<p class="p" >' + "예약 : " + myItem.reservationurl + "</p>";
        }
        ...
        if (myItem.infocenterlodging) {
          output +=
            '<p class="p" >' + "연락처 : " + myItem.infocenterlodging + "</p>";
        }
        ...
        if (myItem.restdateshopping) {
          output +=
            '<p class="p" >' + "휴무일 : " + myItem.restdateshopping + "</p>";
        }
        if (myItem.infocenter) {
          output += '<p class="p" >' + "연락처 : " + myItem.infocenter + "</p>";
        }
        ...
        if (myItem.opentimefood) {
          output +=
            '<p class="p" >' + "운영시간 : " + myItem.opentimefood + "</p>";
        }
        if (myItem.restdatefood) {
          output +=
            '<p class="p" >' + "휴무일 : " + myItem.restdatefood + "</p>";
        }
        if (myItem.infocenterfood) {
          output +=
            '<p class="p" >' + "연락처 : " + myItem.infocenterfood + "</p>";
        }
      }
